...
class Command(BaseCommandCompatible):
    ...
    arguments_dictionary = {
        "--db-version": {"action": "store_true", "dest": "show_ver", "default": False, "help": "Show version info"},
        "--update-version-info": {
            "action": "store",
            "dest": "fetch",
            "type": str,
    ...
    def handle(self, show_ver: bool, fetch: str, **options: Any) -> None:
        ...
        if show_ver:
            ...
            for table in TABLES:
                ...
                print(
                    "Table: {0}, version: {1}, actual: {2}".format(
                        table, ver if ver else "unknown", "No" if ver < latest_version.ver else "Yes"
                    )
                )
